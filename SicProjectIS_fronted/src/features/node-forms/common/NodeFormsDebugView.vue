...
function fieldsForKind(kind?: NodeFormDataKind) {
  switch (kind) {
...
function defaultsForKind(kind?: NodeFormDataKind) {
...
function fieldType(key: string) {
  if (key.startsWith('is') || key.startsWith('has') || key.endsWith('Required') || ['required', 'passed', 'leaderSigned', 'schoolSealed', 'externalSealed', 'removeHighestLowest', 'valid', 'conflictOfInterest'].includes(key)) {
    return 'boolean'
  }
